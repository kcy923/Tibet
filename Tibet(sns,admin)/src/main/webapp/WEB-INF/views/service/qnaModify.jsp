...
<body>
	<form action="${contextPath}/qnaModify.do" id="qnaMod" method="post">
				...
				<div class="form">
					<div class="qna-ask-form">
						<label>제목</label> <input type="text" class="input" id="qna_title"
							name="qna_title" value="" >
					</div>
					<%
					session.getAttribute("memberInfo");
					%>
					<input type="hidden" id="user_id" name="user_id"
						value="${memberInfo.user_id eq user_id == 'admin'}">
					<div class="qna-ask-form">
						<label>공개여부</label> 
					...
					<div class="qna-ask-form">
						<label>비밀번호</label> <input type="password" name="qna_pw"
							class="input" id="password" maxlength="4" value="${qna.qna_pw}"
					...
					<div class="qna-ask-form-img">
						<label>이미지 첨부</label> <input class="qna-ask-form-img-btn"
							type="file" name="qna_img" value="${qna.qna_img}">
					</div>
				</div>
			</div>
			<div class="box-btn">
				<div class="box-btn-text">
					<button type="button" class="btn-text" id="button-reg"
						onclick="javascript:submit_check('${contextPath}/qna.do')">수정</button>
	   ...
	   $('#summernote').summernote({
		...
		function submit_check() {
			...
			if (answer == true) {
				...
				document.getElementById('qnaMod').submit();
				location = url;
			}
		}
